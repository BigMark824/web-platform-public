...
class RouteCompiler implements RouteCompilerInterface
{
    ...
    {
		$slashEscape = fn($str) => str_replace('/', '/+', preg_quote($str));
        $token = $tokens[$index];
        if ('text' === $token[0]) {
            // Text tokens
            return $slashEscape($token[1]);
        } else {
            // Variable tokens
            if (0 === $index && 0 === $firstOptional) {
                // When the only token is an optional variable token, the separator is required
                return sprintf('%s(?P<%s>%s)?', $slashEscape($token[1]), $token[3], $token[2]);
            } else {
                $regexp = sprintf('%s(?P<%s>%s)', $slashEscape($token[1]), $token[3], $token[2]);
